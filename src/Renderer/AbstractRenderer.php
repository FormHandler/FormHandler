...
abstract class AbstractRenderer
{
     ...
     */
    abstract public function render(Element $element): string;
     ...
     * @return string
     */
    protected function getMethodNameForClass(Element $element): string
    {
    ...
    /**
     * Parse the given element and put its attributes in the given tag.
     ...
     * @return string
     */
    protected function parseTag(Tag $tag, Element $element): string
    {
        $list = [
            'disabled'    => 'isDisabled',
            'readonly'    => 'isReadonly',
        ...
        foreach ($list as $attribute => $method) {
            if (method_exists($element, $method)) {
                ...
                if (substr($method, 0, 2) == 'is') {
                    ...
                    $tag->setAttribute($attribute, (string)$value);
                }
            }
        }
        ...
        if (method_exists($element, 'getValue') &&
            ...
            $tag->setAttribute('value', htmlentities((string)$element->getValue(), ENT_QUOTES, 'UTF-8'));
        }
