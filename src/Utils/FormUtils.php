...
class FormUtils
{
     ...
     * @return void
     */
    public static function queryStringToForm(Form $form, array $whitelist = null, array $blacklist = null): void
    {
        if (!empty($_GET)) {
     ...
     * ```
     *
     * As final parameter you can decide if the upload path should be created if it does not exist.
     ...
     *                                          You can also give an umask here (like 644).
     *
     ...
     * @return string
     */
    public static function getNonExistingFilename(string $destination): string
    {
        // find a unique name
    ...
    public static function getMaxUploadSize(): int
    {
        ...
        try {
            $max = FormUtils::sizeToBytes((string)ini_get('upload_max_filesize'));
        } catch (Exception $e) {
        }

        try {
            $max2 = FormUtils::sizeToBytes((string)ini_get('post_max_size'));
            if ($max2 < $max || $max == 0) {
                $max = $max2;
            }
        } catch (Exception $e) {
     ...
     * @throws \Exception
     */
    public static function sizeToBytes(string $str): int
    {
